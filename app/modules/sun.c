...
static int sun_lua_Sun(lua_State* L, int sign, float psunel) {
  ...
  float lon = (float)luaL_checknumber(L, 2);
  float lat = (float)luaL_checknumber(L, 3);
  if (psunel == 0) {sunel = (float)luaL_optnumber(L, 5, 0);} else {sunel = psunel;}
  #endif
  float elevation = (float)luaL_optnumber(L, 4, 0);
...
LROT_BEGIN(sun, NULL, 0)
  // LROT_FUNCENTRY( sin, sun_lua_sin )
  // LROT_FUNCENTRY( asin, sun_lua_asin )
